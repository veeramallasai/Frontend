          ...
          <div className="bg-white p-6 rounded-2xl min-h-[600px] border border-slate-100 shadow-sm">
            ...
            <div className="mb-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <h1 className="text-3xl font-extrabold text-slate-700">My Orders</h1>
              </div>
            </div>

            {/* Filter Pills */}
            <div className="flex flex-wrap gap-3 mb-8 pb-4 border-b border-slate-100">
              <button
                onClick={() => setOrderFilter('all')}
                className={`px-4 py-1.5 rounded-full border text-sm font-bold flex items-center gap-2 transition-all ${
                  orderFilter === 'all'
              ...
              <button
                onClick={() => setOrderFilter('quick')}
                className={`px-4 py-1.5 rounded-full border text-sm font-bold flex items-center gap-1 transition-all ${
                  orderFilter === 'quick'
                    ? 'bg-orange-500 text-white border-orange-500 shadow-xs'
                    : 'border-slate-200 text-slate-700 hover:bg-slate-50'
                }`}
              >
                <span className={orderFilter === 'quick' ? 'text-white' : 'text-orange-500 font-extrabold italic'}>#</span> Quick
              </button>

              <button
                onClick={() => navigate('/customer')}
                className="px-4 py-1.5 rounded-full border border-slate-200 text-sm font-bold text-slate-600 hover:bg-slate-50 transition-all"
              >
                Shop All
              </button>
            ...
            <div className="space-y-6">
              {filteredOrders.length ? filteredOrders.map((order) => {
                const statusStr = String(order.status || 'Processing').toLowerCase();
                const dateStr = String(order.createdAt || '').split(',')[0];
                return (
                  <div key={order.id} className="border-b border-slate-100 pb-6 last:border-0 relative group">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <div className="flex items-center gap-2">
                          <h3 className="text-lg font-extrabold text-slate-800 capitalize">{statusStr}</h3>
                          <span className="text-xs font-bold text-emerald-700 bg-emerald-50 px-2.5 py-0.5 rounded-md border border-emerald-200">
                            #{order.orderNumber || `FTH-${order.id}`}
                          </span>
                        </div>
                        <p className="text-xs text-slate-500 font-semibold mt-1">
                          Placed on {dateStr || new Date().toLocaleDateString()} • Total: <span className="font-extrabold text-slate-900">₹{Number(order.totalAmount || order.total || 0).toFixed(2)}</span>
                        </p>
                      </div>
                      <span className="text-sm font-extrabold text-orange-500 italic">#Quick</span>
                    </div>
                    ...
                    <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6">
                        ...
                        {order.items && order.items.length > 0 ? (
                          order.items.map((item, idx) => (
                            <div key={idx} className="w-[72px] shrink-0 text-center">
                              <div className="w-[72px] h-[72px] rounded-xl border border-slate-200 bg-white p-1 mb-1 flex items-center justify-center overflow-hidden">
                                {item.image ? (
                              ...
                              <p className="text-[10px] text-slate-600 line-clamp-1 font-semibold">{item.productName}</p>
                            </div>
                          ))
                        ) : (
                          <div className="w-[72px] shrink-0 text-center">
                            <div className="w-[72px] h-[72px] rounded-xl border border-slate-200 bg-slate-50 p-1 mb-1 flex items-center justify-center text-2xl text-slate-300">
                              <Package />
                            </div>
                            <p className="text-[10px] text-slate-400 font-semibold">Unknown</p>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex flex-col sm:flex-row items-center gap-4 shrink-0">
                        <div className="bg-[#fef9f2] rounded-lg px-4 py-2 text-xs font-semibold text-slate-600 border border-orange-50/50">
                          Help us improve with your feedback. <span className="text-sky-600 hover:underline cursor-pointer">Rate now</span>
                        </div>
                        <button className="px-8 py-2.5 rounded-full border border-sky-600 text-sky-600 font-bold text-sm hover:bg-sky-50 transition-colors bg-white">
                          Order again
                        </button>
                      </div>
                    </div>
      ...
      case 'cart':
        return (
          <div className="max-w-[1100px] mx-auto py-4">
            <h2 className="text-[28px] font-extrabold text-slate-700 mb-6">Cart</h2>
            ...
            <div className="grid lg:grid-cols-[1.5fr_1fr] gap-6 items-start">
              ...
              <div className="space-y-6">
                    ...
                    <div className="flex justify-between items-center border-b border-slate-100 pb-4 mb-4">
                      <div>
                        <p className="font-extrabold text-[15px] flex items-center gap-1">
                          <span className="text-orange-500 italic">#</span><span className="text-orange-500 italic">Quick</span> <span className="text-slate-700">Delivery</span>
                        </p>
                        <p className="text-[13px] text-slate-500 font-medium mt-0.5">Groceries ({cartCount} item{cartCount !== 1 && 's'})</p>
                      </div>
                      <p className="font-extrabold text-[15px] text-slate-800">₹{cartTotal.toFixed(2)}</p>
                    </div>
                    ...
                    <div className="space-y-4">
                      {cart.length > 0 ? cart.map((item) => (
                        <div key={item.id} className="flex items-center justify-between gap-4">
                          <div className="flex items-center gap-4">
                            <div className="w-[60px] h-[60px] bg-slate-50 rounded-lg border border-slate-100 p-1 flex items-center justify-center shrink-0">
                              {item.image ? (
                            ...
                            <h3 className="text-[14px] font-extrabold text-slate-800 max-w-[180px] leading-tight">{item.name}</h3>
                          </div>
                          
                          <div className="flex items-center gap-6">
                            <div className="flex items-center border border-sky-300 rounded-lg overflow-hidden bg-sky-50 h-8">
                              <button onClick={() => updateCartItem(item.id, item.quantity - 1)} className="px-3 text-sky-600 font-bold hover:bg-sky-100 transition-colors">−</button>
                              <span className="text-[13px] font-bold text-slate-800 w-4 text-center">{item.quantity}</span>
                              <button onClick={() => updateCartItem(item.id, item.quantity + 1)} className="px-3 text-sky-600 font-bold hover:bg-sky-100 transition-colors">+</button>
                            </div>
                            <p className="font-extrabold text-[14px] text-slate-800 w-[60px] text-right">₹{(item.price * item.quantity).toFixed(2)}</p>
                          </div>
                        </div>
                      )) : (
                        <div className="py-8 text-center text-slate-500 font-semibold">Your cart is empty.</div>
                      )}
                    </div>
                  </div>

                  {/* Minimum Order Warning */}
                  {cartTotal > 0 && cartTotal < 99 && (
                    <div className="bg-[#fff4e5] p-5 flex items-center justify-between border-t border-orange-100">
                      <div className="flex items-start gap-3">
                        <div className="w-6 h-6 rounded-full bg-orange-500 text-white font-bold flex items-center justify-center shrink-0 mt-0.5">!</div>
                        <div>
                          <p className="text-[14px] font-semibold text-orange-600">Minimum order value is ₹99</p>
                          <p className="text-[13px] text-slate-600 mt-0.5">Add items worth ₹{(99 - cartTotal).toFixed(2)} from groceries</p>
                        </div>
                      </div>
                      <button className="bg-[#0070a6] hover:bg-[#005e8c] text-white px-5 py-2 rounded-full text-[13px] font-bold transition-colors shadow-sm">
                        Add Items
                      </button>
                    </div>
                  )}
                ...
                {/* You may also like */}
                <div className="bg-[#fcedeb] rounded-2xl p-6 shadow-sm border border-[#fae2df]">
                  <h3 className="text-xl font-extrabold text-slate-900 mb-6">You may also like</h3>
                  <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide snap-x">
                    {products.slice(0, 10).map((product) => (
                      <div key={product.id} className="min-w-[160px] bg-white rounded-xl p-3 shadow-sm border border-slate-100 flex flex-col justify-between shrink-0 snap-start relative group">
                        <button className="absolute top-3 left-3 text-slate-300 hover:text-rose-500 z-10 transition-colors">
                          <Heart className="w-5 h-5 fill-current" />
                        </button>
                        <div className="h-[120px] bg-white rounded-lg flex items-center justify-center mb-3 p-2 relative overflow-hidden group-hover:scale-105 transition-transform duration-300">
                          {product.image ? (
                        ...
                        <div>
                          <h4 className="text-[13px] font-semibold text-slate-700 line-clamp-2 leading-tight mb-2 h-8">{product.name}</h4>
                          <div className="flex items-center justify-between mt-auto">
                            <p className="font-extrabold text-[14px] text-slate-800">₹{product.price}</p>
                            <button 
                              onClick={() => addToCart(product)}
                              className="px-4 py-1.5 border border-[#0070a6] text-[#0070a6] text-[13px] font-bold rounded-lg hover:bg-sky-50 transition-colors"
                            >
                              Add
                            </button>
                          </div>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-white rounded-xl p-5 shadow-sm border border-slate-100 flex flex-col gap-3">
                  <div className="flex justify-between items-center cursor-pointer group">
                    <div className="flex items-center gap-3">
                      <BadgePercent className="w-6 h-6 text-white fill-[#00a152]" />
                      <span className="text-[15px] font-bold text-slate-700 group-hover:text-[#0070a6] transition-colors">No coupons available</span>
                    </div>
                    <ChevronRight className="w-5 h-5 text-slate-400 group-hover:text-[#0070a6]" />
                  </div>
                  <button className="text-[14px] font-bold text-[#0070a6] text-left hover:underline w-max flex items-center gap-1">
                    View all coupons <ChevronRight className="w-4 h-4" />
                  </button>
                </div>
                ...
                <div className="bg-white rounded-xl p-4 shadow-sm border border-slate-100 flex items-center justify-between gap-4">
                  <div className="flex items-start gap-3">
                    ...
                    <p className="text-[13px] font-medium text-slate-700 leading-tight">Enable order updates and important information on WhatsApp</p>
                  </div>
                  <input type="checkbox" className="w-5 h-5 rounded border-slate-300 text-emerald-500 focus:ring-emerald-500 cursor-pointer" />
                </div>

                {/* Payment Details */}
                <div className="bg-white rounded-xl p-5 shadow-sm border border-slate-100 space-y-4">
                  <h3 className="text-[15px] font-extrabold text-slate-800 mb-2">Payment Details</h3>
                  
                  <div className="space-y-3">
                    <div className="flex justify-between text-[14px] font-medium text-slate-700">
                      <span>MRP Total</span>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-[15px] font-extrabold text-slate-800">₹{cartTotal.toFixed(2)}</span>
                    </div>
                  </div>
                  ...
                  <div className="pt-2">
                    <Button 
                      variant="primary" 
                      onClick={() => selectedAddressId ? placeOrder(paymentMethod) : setActiveTab('addresses')}
                      className="w-full bg-[#0070a6] hover:bg-[#005e8c] text-white py-3.5 rounded-[10px] font-bold text-[15px]"
                    >
                      {selectedAddressId ? 'Place Order' : 'Add Address'}
                    </Button>
                    <p className="text-[10px] text-slate-400 mt-4 text-center">Orders are eligible for cancellation or refund only before they are dispatched. View full policy.</p>
                  </div>
                </div>
